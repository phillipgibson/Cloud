...
Now we need to deploy an instance of the Azure Firewall in each regions AKS VNet and configure the NAT rule back to the AKS service internal IP. We will also need to create a Route Table for the AKS cluster subnet to have its default gateway pointed to the internal IP address of the Azure Firewall.
